...
public:
  ...
  // apply matrix
  void apply_matrix(const uint_t iChunk, const reg_t &qubits,
                    const int_t control_bits, const cvector_t<double> &mat,
                    const uint_t gid, const uint_t count) override;

  // apply diagonal matrix
  void apply_diagonal_matrix(const uint_t iChunk, const reg_t &qubits,
                             const int_t control_bits,
                             const cvector_t<double> &diag, const uint_t gid,
                             const uint_t count) override;

  // apply (controlled) X
  void apply_X(const uint_t iChunk, const reg_t &qubits, const uint_t gid,
               const uint_t count) override;

  // apply (controlled) Y
  void apply_Y(const uint_t iChunk, const reg_t &qubits, const uint_t gid,
               const uint_t count) override;
  ...
  void apply_swap(const uint_t iChunk, const reg_t &qubits,
                  const int_t control_bits, const uint_t gid,
                  const uint_t count) override;

  // apply permutation
  void apply_permutation(const uint_t iChunk, const reg_t &qubits,
                         const std::vector<std::pair<uint_t, uint_t>> &pairs,
                         const uint_t gid, const uint_t count) override;

  // apply rotation around axis
  void apply_rotation(const uint_t iChunk, const reg_t &qubits,
                      const Rotation r, const double theta, const uint_t gid,
                      const uint_t count) override;

  // get probabilities of chunk
  void probabilities(std::vector<double> &probs, const uint_t iChunk,
                     const reg_t &qubits) const override;

  // Pauli expectation values
  double expval_pauli(const uint_t iChunk, const reg_t &qubits,
                      const std::string &pauli,
                      const complex_t initial_phase) const override;
};
...
int cuStateVecChunkContainer_MemPoolFree(void *ctx, void *ptr, size_t size,
                                         cudaStream_t stream) {
...
uint_t cuStateVecChunkContainer<data_t>::Allocate(
  ...
  if (err != CUSTATEVEC_STATUS_SUCCESS) {
    ...
    str << "cuStateVecChunkContainer::allocate::custatevecCreate : "
  ...
  status = cudaDeviceGetAttribute(&isMemPoolSupported,
                                  cudaDevAttrMemoryPoolsSupported, idev);
  if (status != cudaSuccess || !isMemPoolSupported) {
    std::stringstream str;
    str << "cuStateVecChunkContainer : cuStateVec support requires memory pool "
           "on GPU device. "
  ...
  status = cudaMemPoolSetAttribute(memory_pool_,
                                   cudaMemPoolAttrReleaseThreshold, &threshold);
  if (status != cudaSuccess) {
    std::stringstream str;
    str << "cuStateVecChunkContainer::cudaMemPoolSetAttribute : "
        << cudaGetErrorString(status);
    throw std::runtime_error(str.str());
  }
  ...
  err = custatevecSetDeviceMemHandler(custatevec_handle_,
                                      &custatevec_mem_handler_);
  if (err != CUSTATEVEC_STATUS_SUCCESS) {
    std::stringstream str;
    str << "cuStateVecChunkContainer::custatevecSetDeviceMemHandler : "
        << custatevecGetErrorString(err);
    throw std::runtime_error(str.str());
  }
  ...
  custatevec_chunk_count_ =
      this->num_chunks_ >> (this->num_pow2_qubits_ - this->chunk_bits_);
...
template <typename data_t>
reg_t cuStateVecChunkContainer<data_t>::sample_measure(
    uint_t iChunk, const std::vector<double> &rnds, uint_t stride, bool dot,
    uint_t count) const {
  if (stride == 1 &&
      count == (1ull << (this->num_qubits_ - this->chunk_bits_))) {
    ...
    err = custatevecSamplerCreate(
        custatevec_handle_, BaseContainer::chunk_pointer(iChunk), state_type,
        this->num_qubits_, &sampler, SHOTS, &extSize);
    if (err != CUSTATEVEC_STATUS_SUCCESS) {
      std::stringstream str;
      str << "cuStateVecChunkContainer::sample_measure : "
    ...
    err = custatevecSamplerPreprocess(custatevec_handle_, sampler, pExtBuf,
                                      extSize);
    if (err != CUSTATEVEC_STATUS_SUCCESS) {
      std::stringstream str;
      str << "cuStateVecChunkContainer::sample_measure : "
    ...
    err =
        custatevecSamplerSample(custatevec_handle_, sampler, &bitStr[0],
                                &bitOrdering[0], this->num_qubits_, &rnds[0],
                                SHOTS, CUSTATEVEC_SAMPLER_OUTPUT_RANDNUM_ORDER);
    if (err != CUSTATEVEC_STATUS_SUCCESS) {
      std::stringstream str;
      str << "cuStateVecChunkContainer::sample_measure : "
...
template <typename data_t>
void cuStateVecChunkContainer<data_t>::apply_matrix(
  ...
  for (int_t i = 0; i < nc; i++) {
    err = custatevecApplyMatrix(
        custatevec_handle_, BaseContainer::chunk_pointer(iChunk) + (i << bits),
        state_type, bits, pMat, CUDA_C_64F, CUSTATEVEC_MATRIX_LAYOUT_COL, 0,
        pQubits, num_qubits, pControl, nullptr, control_bits, comp_type,
        nullptr, 0);
    if (err != CUSTATEVEC_STATUS_SUCCESS) {
      std::stringstream str;
      str << "cuStateVecChunkContainer::apply_matrix : "
          << custatevecGetErrorString(err);
      throw std::runtime_error(str.str());
    }
  }
...
template <typename data_t>
void cuStateVecChunkContainer<data_t>::apply_diagonal_matrix(
    const uint_t iChunk, const reg_t &qubits, const int_t control_bits,
  ...
  custatevecStatus_t err;
  for (int_t i = 0; i < nc; i++) {
    err = custatevecApplyGeneralizedPermutationMatrix(
        custatevec_handle_, BaseContainer::chunk_pointer(iChunk) + (i << bits),
        state_type, bits, nullptr, pMat, CUDA_C_64F, 0, pQubits, num_qubits,
        nullptr, nullptr, 0, nullptr, 0);
    if (err != CUSTATEVEC_STATUS_SUCCESS) {
      std::stringstream str;
      str << "cuStateVecChunkContainer::apply_diagonal_matrix : "
          << custatevecGetErrorString(err);
      throw std::runtime_error(str.str());
    }
  }
...
template <typename data_t>
void cuStateVecChunkContainer<data_t>::apply_X(const uint_t iChunk,
                                               const reg_t &qubits,
                                               const uint_t gid,
                                               const uint_t count) {
  int_t num_qubits = qubits.size();
  ...
  for (int_t i = 0; i < nc; i++) {
    err = custatevecApplyGeneralizedPermutationMatrix(
        custatevec_handle_, BaseContainer::chunk_pointer(iChunk) + (i << bits),
        state_type, bits, &perm[0], nullptr, CUDA_C_64F, 0, pQubits, num_qubits,
        nullptr, nullptr, 0, nullptr, 0);
    if (err != CUSTATEVEC_STATUS_SUCCESS) {
      std::stringstream str;
      str << "cuStateVecChunkContainer::apply_X : "
          << custatevecGetErrorString(err);
      throw std::runtime_error(str.str());
    }
  }
...
template <typename data_t>
void cuStateVecChunkContainer<data_t>::apply_Y(const uint_t iChunk,
                                               const reg_t &qubits,
                                               const uint_t gid,
                                               const uint_t count) {
  int_t num_qubits = qubits.size();
  ...
  for (int_t i = 0; i < nc; i++) {
    err = custatevecApplyGeneralizedPermutationMatrix(
        custatevec_handle_, BaseContainer::chunk_pointer(iChunk) + (i << bits),
        state_type, bits, &perm[0], &diag[0], CUDA_C_64F, 0, pQubits,
        num_qubits, nullptr, nullptr, 0, nullptr, 0);
    if (err != CUSTATEVEC_STATUS_SUCCESS) {
      std::stringstream str;
      str << "cuStateVecChunkContainer::apply_Y : "
          << custatevecGetErrorString(err);
      throw std::runtime_error(str.str());
    }
  }
...
template <typename data_t>
void cuStateVecChunkContainer<data_t>::apply_phase(
    const uint_t iChunk, const reg_t &qubits, const int_t control_bits,
    const std::complex<double> phase, const uint_t gid, const uint_t count) {
...
template <typename data_t>
void cuStateVecChunkContainer<data_t>::apply_swap(const uint_t iChunk,
                                                  const reg_t &qubits,
                                                  const int_t control_bits,
                                                  const uint_t gid,
                                                  const uint_t count) {
  ...
  for (int_t i = 0; i < nc; i++) {
    err = custatevecApplyGeneralizedPermutationMatrix(
        custatevec_handle_, BaseContainer::chunk_pointer(iChunk) + (i << bits),
        state_type, bits, &swap[0], nullptr, CUDA_C_64F, 0, pQubits, num_qubits,
        nullptr, nullptr, 0, nullptr, 0);
    if (err != CUSTATEVEC_STATUS_SUCCESS) {
      std::stringstream str;
      str << "cuStateVecChunkContainer::apply_swap : "
          << custatevecGetErrorString(err);
      throw std::runtime_error(str.str());
    }
  }
...
template <typename data_t>
void cuStateVecChunkContainer<data_t>::apply_permutation(
  ...
  for (int_t i = 0; i < nc; i++) {
    err = custatevecApplyGeneralizedPermutationMatrix(
        custatevec_handle_, BaseContainer::chunk_pointer(iChunk) + (i << bits),
        state_type, bits, perm, nullptr, CUDA_C_64F, 0, pQubits, qubits.size(),
        nullptr, nullptr, 0, nullptr, 0);
    if (err != CUSTATEVEC_STATUS_SUCCESS) {
      std::stringstream str;
      str << "cuStateVecChunkContainer::apply_permutation : "
          << custatevecGetErrorString(err);
      throw std::runtime_error(str.str());
    }
  }
...
template <typename data_t>
void cuStateVecChunkContainer<data_t>::apply_rotation(
  ...
  custatevecStatus_t err;
  for (int_t i = 0; i < nc; i++) {
    err = custatevecApplyPauliRotation(
        custatevec_handle_, BaseContainer::chunk_pointer(iChunk) + (i << bits),
        state_type, bits, -0.5 * theta, &pauli[0], pQubits,
        qubits.size() - control_bits, pControl, nullptr, control_bits);
    if (err != CUSTATEVEC_STATUS_SUCCESS) {
      std::stringstream str;
      str << "cuStateVecChunkContainer::apply_rotation : "
          << custatevecGetErrorString(err);
      throw std::runtime_error(str.str());
    }
  }
...
template <typename data_t>
double cuStateVecChunkContainer<data_t>::norm(uint_t iChunk,
                                              uint_t count) const {
  BaseContainer::set_device();
  ...
  for (int_t i = 0; i < nc; i++) {
    double d;
    err = custatevecAbs2SumArray(
        custatevec_handle_, BaseContainer::chunk_pointer(iChunk) + (i << bits),
        state_type, bits, &d, nullptr, 0, nullptr, nullptr, 0);
    if (err != CUSTATEVEC_STATUS_SUCCESS) {
      std::stringstream str;
      str << "cuStateVecChunkContainer::norm : "
          << custatevecGetErrorString(err);
      throw std::runtime_error(str.str());
    }
    ret += d;
...
template <typename data_t>
void cuStateVecChunkContainer<data_t>::probabilities(
    std::vector<double> &probs, const uint_t iChunk,
  ...
  for (int_t i = 0; i < nc; i++) {
    if (qubits.size() == 1) {
      double p0, p1;
      err = custatevecAbs2SumOnZBasis(
          custatevec_handle_,
          BaseContainer::chunk_pointer(iChunk) + (i << bits), state_type, bits,
          &p0, &p1, &qubits32[0], 1);
      probs[0] += p0;
      probs[1] += p1;
    } else {
    ...
    if (err != CUSTATEVEC_STATUS_SUCCESS) {
      std::stringstream str;
      str << "cuStateVecChunkContainer::probabilities : "
          << custatevecGetErrorString(err);
      throw std::runtime_error(str.str());
    }
  }
...
template <typename data_t>
double cuStateVecChunkContainer<data_t>::expval_pauli(
    const uint_t iChunk, const reg_t &qubits, const std::string &pauli,
  ...
  err = custatevecComputeExpectationsOnPauliBasis(
      custatevec_handle_, BaseContainer::chunk_pointer(iChunk), state_type,
      this->chunk_bits_, ret, pauliOperatorsArray, 1, basisBitsArray,
      nBasisBitsArray);

  if (err != CUSTATEVEC_STATUS_SUCCESS) {
    std::stringstream str;
    str << "cuStateVecChunkContainer::expval_pauli : "
        << custatevecGetErrorString(err);
    throw std::runtime_error(str.str());
  }
