...
template <typename data_t>
uint_t UniversalChunkContainer<data_t>::Allocate(int idev, int chunk_bits,
...
template <typename data_t>
void UniversalChunkContainer<data_t>::allocate_creg(uint_t num_mem,
                                                 uint_t num_reg) {
...
template <typename data_t>
void UniversalChunkContainer<data_t>::calculate_matrix_buffer_size(int bits,
                                                                int shots) {
  uint_t size;
...
template <typename data_t>
void UniversalChunkContainer<data_t>::ResizeMatrixBuffers(int bits,
                                                       int max_shots) {
...
template <typename data_t>
void UniversalChunkContainer<data_t>::StoreMatrix(
...
template <typename data_t>
void UniversalChunkContainer<data_t>::StoreMatrix(const std::complex<double> *mat,
                                               uint_t iChunk,
                                               uint_t size) const {
  set_device();
...
template <typename data_t>
void UniversalChunkContainer<data_t>::StoreUintParams(
...
template <typename data_t>
void UniversalChunkContainer<data_t>::CopyIn(Chunk<data_t> &src, uint_t iChunk) {
  ...
  if (err != cudaSuccess) {
    std::stringstream str;
    str << "UniversalChunkContainer::CopyIn : " << cudaGetErrorName(err);
    throw std::runtime_error(str.str());
  }
...
template <typename data_t>
void UniversalChunkContainer<data_t>::CopyOut(Chunk<data_t> &dest, uint_t iChunk) {
  uint_t size = 1ull << this->chunk_bits_;
  ...
  if (err != cudaSuccess) {
    std::stringstream str;
    str << "UniversalChunkContainer::CopyOut " << dest.pointer() << ","
...
void UniversalChunkContainer<data_t>::CopyIn(thrust::complex<data_t> *src,
  ...
  if (this_size < size) {
    std::stringstream str;
    str << "UniversalChunkContainer::CopyIn chunk size " << this_size
        << " is less than " << size;
    throw std::runtime_error(str.str());
  }
...
void UniversalChunkContainer<data_t>::CopyOut(thrust::complex<data_t> *dest,
  ...
  if (this_size < size) {
    std::stringstream str;
    str << "UniversalChunkContainer::CopyOut chunk size " << this_size
        << " is less than " << size;
    throw std::runtime_error(str.str());
  }
  synchronize(iChunk);
...
void UniversalChunkContainer<data_t>::Swap(Chunk<data_t> &src, uint_t iChunk,
  ...
  if (err != cudaSuccess) {
    std::stringstream str;
    str << "UniversalChunkContainer::Swap : " << cudaGetErrorName(err);
    throw std::runtime_error(str.str());
  }
...
// set qubits to be blocked
template <typename data_t>
void UniversalChunkContainer<data_t>::set_blocked_qubits(uint_t iChunk,
                                                      const reg_t &qubits) {
...
// queue gate for blocked execution
template <typename data_t>
void UniversalChunkContainer<data_t>::queue_blocked_gate(
...
// do all gates stored in queue
template <typename data_t>
void UniversalChunkContainer<data_t>::apply_blocked_gates(uint_t iChunk) {
  if (num_matrices_ == 1 && iChunk > 1 && iChunk < this->num_chunks_) {
...
template <typename data_t>
void UniversalChunkContainer<data_t>::copy_to_probability_buffer(
...
template <typename data_t>
void UniversalChunkContainer<data_t>::copy_reduce_buffer(std::vector<double> &ret,
                                                      uint_t iChunk,
