...
public:
  ...
  std::complex<data_t> *data() const {
    return (std::complex<data_t> *)chunk_.pointer();
  }
  ...
  bool top_of_group() { return (chunk_.pos() == 0); }
  ...
  void initialize_from_data(const std::complex<data_t> *data,
  ...
  void
  apply_permutation_matrix(const reg_t &qubits,
  ...
  // swap between chunk
  void apply_chunk_swap(const reg_t &qubits, QubitVectorThrust<data_t> &chunk,
                        bool write_back = true);
  void apply_chunk_swap(const reg_t &qubits, uint_t remote_chunk_index);

  void apply_chunk_swap(QubitVectorThrust<data_t> &chunk, uint_t dest_offset,
                        uint_t src_offset, uint_t size);
  ...
  virtual void apply_roerror(const Operations::Op &op,
  ...
  virtual void apply_batched_reset(const reg_t &qubits,
                                   std::vector<RngEngine> &rng);
...
protected:
  ...
  int num_threads_per_group_ =
      1; // number of threads per GPU for multi-chunks/shots
  ...
  template <typename Function>
  void apply_function(Function func,
                      const std::vector<std::complex<double>> &mat,
                      const std::vector<uint_t> &prm, uint_t count = 0) const;

  template <typename Function>
  void apply_function_sum(double *pSum, Function func,
                          bool async = false) const;

  template <typename Function>
  void apply_function_sum2(double *pSum, Function func,
                           bool async = false) const;
...
void QubitVectorThrust<data_t>::check_qubit(const uint_t qubit) const {
  if (qubit + 1 > num_qubits_) {
    std::string error = "QubitVectorThrust: qubit index " +
                        std::to_string(qubit) + " > " +
...
template <typename data_t>
void QubitVectorThrust<data_t>::check_matrix(const cvector_t<data_t> &vec,
                                             uint_t nqubits) const {
  const size_t DIM = 1ull << nqubits;
  const auto SIZE = vec.size();
  if (SIZE != DIM * DIM) {
    std::string error = "QubitVectorThrust: vector size is " +
                        std::to_string(SIZE) +
...
template <typename data_t>
void QubitVectorThrust<data_t>::check_vector(const cvector_t<data_t> &vec,
                                             uint_t nqubits) const {
  const size_t DIM = 1ull << nqubits;
  const auto SIZE = vec.size();
  if (SIZE != DIM) {
    std::string error = "QubitVectorThrust: vector size is " +
                        std::to_string(SIZE) + " != " + std::to_string(DIM);
    throw std::runtime_error(error);
  }
}
...
void QubitVectorThrust<data_t>::check_checkpoint() const {
  if (!checkpoint_) {
    throw std::runtime_error("QubitVectorThrust: checkpoint must exist for "
                             "inner_product() or revert()");
  }
}
...
thrust::complex<data_t> &QubitVectorThrust<data_t>::operator[](uint_t element) {
  ...
  if (element > data_size_) {
    std::string error = "QubitVectorThrust: vector index " +
...
QubitVectorThrust<data_t>::operator[](uint_t element) const {
  // Error checking
  if (element > data_size_) {
    std::string error = "QubitVectorThrust: vector index " +
                        std::to_string(element) + " > " +
...
QubitVectorThrust<data_t>::copy_to_vector() const {
  ...
  return AER::Vector<std::complex<data_t>>::copy_from_buffer(data_size_,
                                                             &ret[0]);
}
...
//------------------------------------------------------------------------------
template <typename data_t>
void QubitVectorThrust<data_t>::initialize_component(
    const reg_t &qubits, const cvector_t<double> &state0) {
  if (qubits.size() == 1) {
...
#ifdef AER_DEBUG
  if (chunk_.pos() == 0) {
    spdlog::debug(" ==== Thrust qubit vector initialization {} qubits ====",
                  num_qubits_);
    if (chunk_.device() >= 0)
      spdlog::debug("    TEST [id={}]: device = {}, pos = {}, place = {} / {}",
                    chunk_index_, chunk_.device(), chunk_.pos(), chunk_.place(),
                    chunk_manager_->num_places());
    else
      spdlog::debug("    TEST [id={}]: allocated on host (place = {})",
                    chunk_index_, chunk_.place());
  }
#endif
}
...
void QubitVectorThrust<data_t>::checkpoint() {
  ...
  chunk_.CopyOut(
      (thrust::complex<data_t> *)thrust::raw_pointer_cast(checkpoint_.data()),
      data_size_);
...
std::complex<double> QubitVectorThrust<data_t>::inner_product() const {
  ...
  if (num_qubits_ > omp_threshold_ && omp_threads_ > 1)
    ...
    dot = thrust::inner_product(thrust::seq, vec0, vec0 + data_size_ * 2, vec1,
                                0.0);
#endif
...
template <typename data_t>
thrust::complex<data_t> *
QubitVectorThrust<data_t>::send_buffer(uint_t &size_in_byte) {
  thrust::complex<data_t> *pRet;

#ifdef AER_DISABLE_GDR
  if (chunk_.device() < 0) {
    pRet = chunk_.pointer();
  } else { // if there is no GPUDirectRDMA support, copy chunk on CPU before
           // using MPI
    pRet = nullptr;
    if (chunk_manager_->MapBufferChunkOnHost(send_chunk_)) {
      ...
      throw std::runtime_error(
          "QubitVectorThrust: send buffer can not be allocated");
    }
  }
#else
  pRet = chunk_.pointer();
...
template <typename data_t>
thrust::complex<data_t> *
QubitVectorThrust<data_t>::recv_buffer(uint_t &size_in_byte) {
  ...
  if (!recv_chunk_.is_mapped()) {
    throw std::runtime_error(
        "QubitVectorThrust: receive buffer can not be allocated");
  }
  chunk_.synchronize();
...
void QubitVectorThrust<data_t>::initialize() {
  ...
  if (multi_chunk_distribution_) {
    if (chunk_index_ == 0) {
      apply_function(Chunk::initialize_kernel<data_t>(
          t, chunk_manager_->chunk_bits(),
          (1ull << chunk_manager_->num_qubits())));
    } else {
      zero();
    }
    chunk_.synchronize();
    ...
    apply_function(Chunk::initialize_kernel<data_t>(
        t, chunk_manager_->chunk_bits(),
        (1ull << chunk_manager_->chunk_bits())));
  }
...
void QubitVectorThrust<data_t>::initialize_from_vector(const list_t &statevec) {
  if (data_size_ < statevec.size()) {
    std::string error =
        "QubitVectorThrust::initialize input vector is incorrect length (" +
        std::to_string(data_size_) + "!=" + std::to_string(statevec.size()) +
        ")";
    throw std::runtime_error(error);
  }
...
template <typename data_t>
void QubitVectorThrust<data_t>::initialize_from_vector(
    const std::vector<std::complex<data_t>> &vec) {
  initialize_from_data(&vec[0], vec.size());
}

template <typename data_t>
void QubitVectorThrust<data_t>::initialize_from_vector(
    const AER::Vector<std::complex<data_t>> &vec) {
  initialize_from_data(vec.data(), vec.size());
}

template <typename data_t>
void QubitVectorThrust<data_t>::initialize_from_data(
    const std::complex<data_t> *statevec, const size_t num_states) {
...
template <typename data_t>
void QubitVectorThrust<data_t>::initialize_creg(
    uint_t num_memory, uint_t num_register, const std::string &memory_hex,
    const std::string &register_hex) {
  num_creg_bits_ = num_register;
  num_cmem_bits_ = num_memory;
  if (chunk_manager_) {
...
template <typename data_t>
template <typename Function>
void QubitVectorThrust<data_t>::apply_function(Function func,
                                               uint_t count) const {
...
template <typename data_t>
template <typename Function>
void QubitVectorThrust<data_t>::apply_function(
    Function func, const std::vector<std::complex<double>> &mat,
    const std::vector<uint_t> &prm, uint_t count) const {
  uint_t chunk_count = count;
  if (chunk_count == 0) {
    if (!cuStateVec_enable_ && func.batch_enable() &&
        ((multi_chunk_distribution_ && chunk_.device() >= 0) ||
...
template <typename data_t>
void QubitVectorThrust<data_t>::apply_multiplexer(
...
template <typename data_t>
void QubitVectorThrust<data_t>::apply_diagonal_matrix(
...
template <typename data_t>
void QubitVectorThrust<data_t>::apply_permutation_matrix(
    const reg_t &qubits, const std::vector<std::pair<uint_t, uint_t>> &pairs) {
  uint_t count = get_chunk_count();
  if (count == 0)
    return;
...
void QubitVectorThrust<data_t>::apply_chunk_swap(const reg_t &qubits,
  ...
  if (q0 >= num_qubits_) { // exchange whole of chunk each other
    ...
    if (bufferChunk.is_mapped()) {
      bufferChunk.Execute(Chunk::CSwapChunk_func<data_t>(
                              qubits, num_qubits_, pChunk0, pChunk1, true),
                          1);
      ...
      chunk_.Execute(Chunk::CSwapChunk_func<data_t>(qubits, num_qubits_,
                                                    pChunk0, pChunk1, true),
                     1);
      chunk_.synchronize();
    }
  }
...
void QubitVectorThrust<data_t>::apply_chunk_swap(const reg_t &qubits,
  ...
  if (q0 >= num_qubits_) { // exchange whole of chunk each other
...
#ifdef AER_DISABLE_GDR
    if (chunk_.device() >=
        0) { // if there is no GPUDirectRDMA support, copy chunk from CPU
    ...
    chunk_.Execute(Chunk::CSwapChunk_func<data_t>(qubits, num_qubits_, pLocal,
                                                  pRemote, false),
                   1);
...
// swap part of chunks
template <typename data_t>
void QubitVectorThrust<data_t>::apply_chunk_swap(QubitVectorThrust<data_t> &src,
                                                 uint_t dest_offset,
                                                 uint_t src_offset,
                                                 uint_t size) {
...
template <typename data_t>
void QubitVectorThrust<data_t>::apply_mcphase(
    const reg_t &qubits, const std::complex<double> phase) {
  uint_t count = get_chunk_count();
  if (count == 0)
    return;
  ...
  if (mat[1] == 0.0 && mat[2] == 0.0) {
    ...
    if (N == 1) {
      ...
      if (register_blocking_) {
        ...
        chunk_.queue_blocked_gate('d', qubits[qubits.size() - 1], mask,
                                  &diag[0]);
      } else {
        uint_t count = get_chunk_count();
        if (count == 0)
    ...
    if (N == 1) {
      ...
      if (register_blocking_) {
        ...
        chunk_.queue_blocked_gate('u', qubits[qubits.size() - 1], mask,
                                  &mat[0]);
      } else {
        uint_t count = get_chunk_count();
        if (count == 0)
...
template <typename data_t>
void QubitVectorThrust<data_t>::apply_rotation(const reg_t &qubits,
                                               const Rotation r,
                                               const double theta) {
  uint_t count = get_chunk_count();
  if (count == 0)
...
template <typename data_t>
void QubitVectorThrust<data_t>::apply_diagonal_matrix(
    const uint_t qubit, const cvector_t<double> &diag) {
  uint_t count = get_chunk_count();
  if (count == 0)
    return;
...
template <typename data_t>
std::vector<double>
QubitVectorThrust<data_t>::probabilities(const reg_t &qubits) const {
  const size_t N = qubits.size();
  const int_t DIM = 1 << N;
...
public:
  reset_after_measure_func(int nq, double *probs, uint_t prob_size,
                           uint_t iter) {
    num_qubits_ = nq;
    probs_ = probs;
  ...
  const char *name(void) { return "reset_after_measure"; }
};

template <typename data_t>
class set_probability_buffer_for_reset_func
    : public Chunk::GateFuncBase<data_t> {
protected:
...
public:
  set_probability_buffer_for_reset_func(double *probs, uint_t prob_size,
                                        double *reduce, uint_t red_size) {
    probs_ = probs;
    reduce_ = reduce;
    prob_buf_size_ = prob_size;
  ...
  const char *name(void) { return "set_probability_buffer_for_reset"; }
};
...
public:
  check_measure_probability_func(int nq, double *probs, uint_t prob_size,
                                 double *reduce, uint_t red_size, uint_t iter,
                                 uint_t nm, uint_t nr) {
    num_qubits_ = nq;
    probs_ = probs;
    reduce_ = reduce;
  ...
  __host__ __device__ void operator()(const uint_t &i) const {
    ...
    if (reset) {
      ...
      if (bit == iter_) { // applying only for iter != last, last case will be
                          // aplied out of loop
        // set system register[0] to 0 not to execute measure/reset kernels
        // anymore
        i64 = sreg >> 6;
        ibit = sreg & 63;
        this->cregs_[iChunk * n64 + i64] &= (~(1ull << ibit));
  ...
  const char *name(void) { return "check_measure_probability"; }
};
...
void QubitVectorThrust<data_t>::apply_batched_measure(
  ...
  apply_function(set_probability_buffer_for_reset_func<data_t>(
      chunk_.probability_buffer(), chunk_.container()->num_chunks(),
  ...
  for (i = 0; i < DIM - 1; i++) {
    chunk_.set_conditional(system_reg);
    apply_function_sum(nullptr, Chunk::probability_func<data_t>(qubits, i),
                       true);

    apply_function(check_measure_probability_func<data_t>(
        qubits.size(), chunk_.probability_buffer(),
        chunk_.container()->num_chunks(), chunk_.reduce_buffer(),
        chunk_.reduce_buffer_size(), i, cmemory.size(), cregs.size()));

    chunk_.set_conditional(system_reg + 1);
    apply_function(reset_after_measure_func<data_t>(
        qubits.size(), chunk_.probability_buffer(),
        chunk_.container()->num_chunks(), i));
    store_cregister(system_reg + 1, 0);
  }
  // for last case
  ...
  apply_function(reset_after_measure_func<data_t>(
      qubits.size(), chunk_.probability_buffer(),
      chunk_.container()->num_chunks(), DIM - 1));
...
public:
  ...
  const char *name(void) { return "reset"; }
};
...
void QubitVectorThrust<data_t>::apply_batched_reset(
  ...
  for (i = 0; i < DIM - 1; i++) {
    ...
    apply_function(check_measure_probability_func<data_t>(
        qubits.size(), chunk_.probability_buffer(),
        chunk_.container()->num_chunks(), chunk_.reduce_buffer(),
        chunk_.reduce_buffer_size(), i, 0, 0));

    chunk_.set_conditional(system_reg + 1);
    apply_function(reset_func<data_t>(qubits.size(),
                                      chunk_.probability_buffer(),
                                      chunk_.container()->num_chunks(), i));
    store_cregister(system_reg + 1, 0);
  }
  chunk_.keep_conditional(false);
  chunk_.set_conditional(system_reg);
  apply_function(reset_func<data_t>(qubits.size(), chunk_.probability_buffer(),
                                    chunk_.container()->num_chunks(), DIM - 1));
}
...
public:
  ...
  __host__ __device__ void operator()(const uint_t &i) const {
    ...
    ibit = reg_set_ & 63;
    this->cregs_[iChunk * n64 + i64] =
        (this->cregs_[iChunk * n64 + i64] & (~(1ull << ibit))) | (val_ << ibit);
  }
  const char *name(void) { return "set_creg"; }
};
...
public:
  ...
  __host__ __device__ void operator()(const uint_t &i) const {
    ...
    ibit = reg_set_ & 63;
    this->cregs_[iChunk * n64 + i64] =
        (this->cregs_[iChunk * n64 + i64] & (~(1ull << ibit))) | (val << ibit);
  }
  const char *name(void) { return "set_batched_creg"; }
};

template <typename data_t>
int_t QubitVectorThrust<data_t>::set_batched_system_conditional(int_t src_reg,
                                                                reg_t &mask) {
  int_t sys_reg = num_creg_bits_ + num_cmem_bits_ + QV_NUM_INTERNAL_REGS -
                  1; // use last internal reg
...
public:
  ...
  const char *name(void) { return "copy_creg"; }
};
...
template <typename data_t>
double
QubitVectorThrust<data_t>::expval_pauli(const reg_t &qubits,
...
double QubitVectorThrust<data_t>::expval_pauli(
    ...
    const QubitVectorThrust<data_t> &pair_chunk, const uint_t z_count,
    const uint_t z_count_pair, const complex_t initial_phase) const {
  uint_t x_mask, z_mask, num_y, x_max;
  std::tie(x_mask, z_mask, num_y, x_max) = pauli_masks_and_phase(qubits, pauli);
  ...
  apply_function_sum(
      &ret, Chunk::expval_pauli_inter_chunk_func<data_t>(
                x_mask, z_mask, phase, pair_ptr, z_count, z_count_pair));
...
void QubitVectorThrust<data_t>::apply_pauli(const reg_t &qubits,
  ...
  if (x_mask == 0) {
    ...
    apply_function(
        Chunk::multi_pauli_func<data_t>(x_mask, z_mask, x_max, phase));
  }
}
...
public:
  ...
  const char *name(void) { return "batched_pauli"; }
};
...
void QubitVectorThrust<data_t>::apply_batched_pauli_ops(
  ...
  auto count_paulis = [this, &params, ops](int_t i) {
    ...
    for (j = 0; j < ops[i].size(); j++) {
      ...
      if (ops[i][j].name == "x") {
        ...
        uint_t pauli_x_mask = 0, pauli_z_mask = 0, pauli_num_y = 0,
               pauli_x_max = 0;
        std::tie(pauli_x_mask, pauli_z_mask, pauli_num_y, pauli_x_max) =
            pauli_masks_and_phase(ops[i][j].qubits, ops[i][j].string_params[0]);
  ...
  Utils::apply_omp_parallel_for((num_inner_threads > 1), 0, count, count_paulis,
                                num_inner_threads);
...
public:
  MatrixMult2x2_conditional(const cvector_t<double> &mat, int q, double *probs,
                            uint_t prob_size) {
    m0 = mat[0];
    m1 = mat[1];
    m2 = mat[2];
  ...
  __host__ __device__ void operator()(const uint_t &i) const {
    ...
    uint_t iChunk = i >> this->chunk_bits_;
    double scale =
        1.0 / sqrt(probs_[iChunk + QV_RESET_CURRENT_PROB * prob_buf_size_]);
  ...
  const char *name(void) { return "MatrixMult2x2_conditional"; }
};
...
public:
  MatrixMultNxN_conditional(uint_t nq, double *probs, uint_t prob_size)
      : Chunk::GateFuncWithCache<data_t>(nq) {
    probs_ = probs;
    prob_buf_size_ = prob_size;
  }
  ...
  const char *name(void) { return "multNxN_conditional"; }
};
...
public:
  check_kraus_probability_func(double *probs, uint_t prob_size, double *reduce,
                               uint_t red_size) {
    probs_ = probs;
    reduce_ = reduce;
    prob_buf_size_ = prob_size;
  ...
  const char *name(void) { return "check_kraus_probability"; }
};

template <typename data_t>
void QubitVectorThrust<data_t>::apply_batched_kraus(
  ...
  // total probability
  apply_function(set_probability_buffer_for_reset_func<data_t>(
      chunk_.probability_buffer(), chunk_.container()->num_chunks(), nullptr,
      0));
  ...
  if (N == 1) {
    for (i = 0; i < kmats.size(); i++) {
      ...
      chunk_.set_conditional(system_reg);
      apply_function_sum(
          nullptr, Chunk::NormMatrixMult2x2<data_t>(vmat, qubits[0]), true);

      apply_function(check_kraus_probability_func<data_t>(
          chunk_.probability_buffer(), chunk_.container()->num_chunks(),
          chunk_.reduce_buffer(), chunk_.reduce_buffer_size()));

      // multiply only when system reg[1] is 1
      chunk_.set_conditional(system_reg + 1);
      apply_function(MatrixMult2x2_conditional<data_t>(
          vmat, qubits[0], chunk_.probability_buffer(),
          chunk_.container()->num_chunks()));
      store_cregister(system_reg + 1, 0);
    }
  } else {
    auto qubits_sorted = qubits;
    ...
    for (i = 0; i < kmats.size(); i++) {
      ...
      // multiply only when system reg[1] is 1
      chunk_.set_conditional(system_reg + 1);
      apply_function(MatrixMultNxN_conditional<data_t>(
          N, chunk_.probability_buffer(), chunk_.container()->num_chunks()));
      store_cregister(system_reg + 1, 0);
    }
  }
  chunk_.set_conditional(-1);
...
public:
  ...
  // in uint param array: array of qubits results to be stored, mask registers,
  // target registers
  __host__ __device__ void operator()(const uint_t &i) const {
    uint_t iChunk = i;
  ...
  const char *name(void) { return "bfunc_kernel"; }
};
...
void QubitVectorThrust<data_t>::apply_bfunc(const Operations::Op &op) {
  ...
  n64 = (num_creg_bits_ + num_cmem_bits_ + QV_NUM_INTERNAL_REGS + 63) >>
        6; // number of 64-bit integer
  ...
  for (iparam = 0; iparam < 2; iparam++) {
    ...
    for (i = added; i < n64;
         i++) { // pack 0 if there is not enough values in string_params
      val = 0;
      params.push_back(val);
    }
  }
...
public:
  ...
  const char *name(void) { return "roerror_kernel"; }
};

template <typename data_t>
void QubitVectorThrust<data_t>::apply_roerror(const Operations::Op &op,
                                              std::vector<RngEngine> &rng) {
  if (((multi_chunk_distribution_ && chunk_.device() >= 0) || enable_batch_) &&
      chunk_.pos() != 0)
    return; // first chunk execute all in batch
  ...
  apply_function(roerror_kernel<data_t>(op.memory.size(), num_creg_bits_,
                                        chunk_.probability_buffer()));
...
template <typename data_t>
void QubitVectorThrust<data_t>::DebugMsg(const char *str,
...
void QubitVectorThrust<data_t>::DebugMsg(const char *str,
  ...
  spdlog::debug(" [{0}] {1} : {2:e}, {3:e} ", debug_count++, str, std::real(c),
                imag(c));
}
...
inline std::ostream &operator<<(std::ostream &out,
                                const AER::QV::QubitVectorThrust<data_t> &qv) {
